...
interface LoginPageProps {
  onLogin: (username: string, password: string) => Promise<{ success: boolean; error?: string; mustChangePassword?: boolean }>;
  onBack: () => void;
}
...
export function LoginPage({ onLogin, onBack }: LoginPageProps) {
  ...
  // Admin gate for demo credentials
  ...
  return (
    <div className="min-h-screen bg-[#0a0e1a] text-white font-['Inter',sans-serif] flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="relative w-full max-w-md">
        ...
        {/* Demo Credentials Toggle — Requires admin auth */}
        <div className="mt-6">
          <button
            type="button"
            onClick={() => {
            ...
            className="w-full flex items-center justify-center gap-2 text-xs text-white/20 hover:text-white/40 transition py-2"
          >
            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
            </svg>
            {showDemoCredentials ? 'Ocultar acceso rápido' : 'Acceso de administración'}
          </button>

          {/* Admin Gate - Password prompt */}
          {showAdminGate && !showDemoCredentials && (
            <div className="mt-2 bg-yellow-500/5 border border-yellow-500/15 rounded-2xl p-5 animate-[fadeIn_0.3s_ease-out]">
              <div className="flex items-center gap-2 mb-3">
                <svg className="w-4 h-4 text-yellow-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
                </svg>
                <span className="text-xs font-semibold text-yellow-400">Verificación de administrador requerida</span>
              </div>

              {adminError && (
                <div className="flex items-center gap-2 p-2.5 mb-3 bg-red-500/10 border border-red-500/20 rounded-lg">
                  <svg className="w-4 h-4 text-red-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                  <span className="text-[11px] text-red-300">{adminError}</span>
                </div>
              )}
              ...
              <div className="space-y-3">
                <input
                  ...
                  className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-xs text-white focus:border-yellow-500 outline-none transition placeholder:text-white/20"
                />
                <input
                  type="password"
                  value={adminPass}
                  ...
                  onKeyDown={(e) => {
                    if (e.key === 'Enter') {
                      if (adminUser === 'admin' && adminPass === '23784685') {
                ...
                <button
                  ...
                  className="w-full py-2.5 bg-yellow-600/20 hover:bg-yellow-600/30 border border-yellow-500/20 rounded-xl text-xs font-semibold text-yellow-400 transition"
                >
                  Verificar Acceso
                </button>
              </div>
          ...
          {/* Demo credentials — only shown after admin verification */}
          {showDemoCredentials && (
            <div className="mt-2 bg-blue-500/5 border border-blue-500/10 rounded-2xl p-5 animate-[fadeIn_0.3s_ease-out]">
              <div className="flex items-center gap-2 mb-3">
              ...
              <div className="space-y-2.5">
                {[
                  { user: 'admin', pass: 'admin123', role: 'Administrador', desc: 'Acceso total · Ve todos los reportes · Descargas', icon: '🛡️' },
                  { user: 'coord.rodriguez', pass: 'coord123', role: 'Coordinador', desc: 'Gestión usuarios · Reportes · Gráficas · Descargas', icon: '📋' },
                  { user: 'dra.gonzalez', pass: 'doctor123', role: 'Doctor', desc: 'Debe cambiar clave · Solo sus reportes', icon: '🩺' },
                  { user: 'enf.perez', pass: 'enfermero123', role: 'Enfermero', desc: 'Debe cambiar clave · Solo sus reportes', icon: '💉' },
                ].map(cred => (
                  <button
                    key={cred.user}
                    type="button"
        ...
        <p className="text-center text-[11px] text-white/15 mt-6">
          Contacte al administrador para obtener una cuenta de usuario
        </p>
      </div>
    </div>
  );
